...
\begin{abstract}
This study examines the use of the GRASS GIS for raster image analysis with aim to perform topographic and environmental analysis in eastern China, Shandong Province, Qingdao area. The variables include land surface temperature assessed by correlation between the Landsat ETM+ channels and colour composites. The methodology is based on the scripting approach of GRASS GIS and its modules. The data include two satellite Landsat TM images with 5-years span (19 May 2009 and September 10 2004) captured from the GloVis USGS. These images were imported to GRASS GIS using GDAL library by ‘r.in.gdal’ module. The topography (DEM) was assessed by GRASS GIS ‘d.rast’ module with isolines plotted using 'r.contour ' and 'd.vect'. The map elements were displayed by 'v.in.region', 'd.vect', 'v.info'. The slope and aspect maps based on DEM were plotted using module ‘r.slope.aspect’ and visualised by consecutive combination of ‘d.rast’, ‘r.colors’, ‘d.legend’, ‘d.text’ and ‘d.grid’ modules. The scatterplot of pairwise correlation of the Landsat TM7 channels (10 and 20, 50 and 70) was performed using ‘d.correlate’. The applied code from the ‘r.mapcalc’ was used for image calibration from the DN to apparent radiance at sensor. Two maps showing land surface temperature for 2009 and 2004 were plotted by ‘r.mapcalc’. The land surface temperature maps show lower values over water areas and forests and higher values in urban areas. Plotting land surface temperature map was made to analyse variations in climate settings in Shandong region based on comparative analysis. The ‘r.mapcalc’ module was used to map changes in land surface temperature during 5 years. The images were visualised by natural and false colour composites of bands. The images were plotted by ‘d.rgb’ module using following band Landsat combinations: 1) colour infrared with focus on vegetation (5-4-3); 2) natural colour composites (1-2-3); 3) contrasting slopes for topography (4-5-7); 4) vegetation analysis (2-3-4); 5) focus on land/water borders (5-6-7). The study demonstrated cartographic functionality of the GRASS GIS specifically for remote sensing data processing and topographic analysis. The environmental modelling shown variations in land surface temperature, slope steepness and orientation (aspect). The GRASS GIS demonstrated to be an effective and functional tool for remote sensing.
\end{abstract}
...
The use of GRASS GIS functionality for automated data processing through shell scripting enables to describe and visualise geographic phenomena, which finally contributes to our understanding of the geographic patterns and processes, as well as estimate the results of the environmental change through technical cartographic support \citep{Lemenkova202032}. Hence, current research presents GRASS GIS application with its modular structure for processing and visualisation of the multi-temporal Landsat TM images aimed at environmental analysis.

Scripting-based GRASS GIS provide an exceptional opportunity to visualise geospatial data and investigate how environmental parameters vary in space and time. For example, visialising geospatial data can enable discover associations between environmental variables and topography and point at potential links. Intensive urbanisation and agricultural activities reflect both environmental and anthropogenic aspects of land cover changes. For example, urban areas generally have higher land surface temperature, while natural land cover types (bare land, forests and water areas) mostly have lower land surface temperature. To find out the effect of urbanisation on land cover changes and climate change reflected in land surface temperature, this study used special modules of GRASS GIS for processing raster datasets by scripts.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage
\section{Approach}\label{Approach}
\subsection{Novelty}
The machine learning in cartography, especially for data visualisation, has become state of the art for mapping techniques and spatial data analysis. However, the applicability and acceptance of scripting approaches specifically in GRASS GIS are mostly delayed compared to the traditional GUI-based GIS. Even in GRASS GIS cases, this software is more often used in cases for geomorphologic analysis and vector data mapping rather than remote sensing. For that purpose, Landsat TM imagery datasets were analysed by GRASS GIS with aim to perform raster data analysis using functionality of raster computations with a case study of NE China, Shandong Province. 
...
Geographically, the Shandong Province of China is represented by the dominated semi- and temperature monsoon climate marked with seasonal variations, favourable for planting agricultural areas of different crop types. The agriculture in eastern China includes a wide variety of species  on the yellow-soil plains, including, e.g. sorghum \citep{su10103428}, fruits, ornamental plants, vegetables \citep{WANG2020881}, tobacco \citep{LIU2021144879}, wine, cotton (\cite{PEMSL200728}, \cite{LI2021126750}) and maize (\cite{YU2021106649}, \cite{ZOU2021126304}). The environmental of easter China currently experiences pressure from the agricultural activities, as well as environmental and climate fluctuations. 

This study estimated the topographic and environmental parameters in the Shandong Province of China retrieved from the satellite images by GRASS GIS scripting. Specifically, the study presented analysis of the terrain curvature, steepness and orientation (slope and aspect) and land surface temperature. All these variables are important parameters for agricultural and environmental assessment. Among others, land surface temperature is an important indicator in regulation of temperature and surface processes on Earth in regional or global scales \citep{WORKU2021100520}. Besides, land surface temperature is also an important characteristics for the energy interchange exchange between the surface and the atmosphere in the cities and tree canopies, as heat waves and thermal balance of an area \citep{JUNG2021126930}. Together with vegetation cover and canopy health, land surface temperature is an important indicator of landscape health \citep{JAMALI2022113970}. 

Thus, the relationship between the land surface temperature, topographic elevation, vegetation, and urbanisation are the key factors for activities on landscape conservation and monitoring. Land surface temperature is an important factor for urban environmental and climate studies showing the complexity of relationships between the urban structure components, such as concrete and grass, large areas of asphalt and green vegetation in urban parks, etc \citep{ARMSON2012245}. It is an indispensable  component in analysis of droughts, flash floods or environmental monitoring and assessment in urban studies \citep{Asgarian}. At the same time, advancement in remote sensing enabled the technical possibility for the use of remote sensing data to evaluate land surface temperature as information derived from the satellite images (\cite{6626038}, \cite{Cook}, \cite{Hulley}, \cite{XIAO200669}). This study considers the existing applications on environmental monitoring using remote sensing data for the case study of eastern China. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Methods and Samples}\label{Methods}
The methodology is based on using GRASS GIS, version 7.6 \cite{Neteler2000}, an open source GIS developed since 1982 and continuously updated. The GRASS (Geographical Resources Analysis Support System) GIS, was selected as a main software in this research. The GRASS is a multipurpose GIS, with data organised as raster and vector maps, providing a wide range of tools to support most of the GIS functionality \cite{NetelerMitasova2008}. Each GRASS GIS module consists of a pattern and functions with an associated action of raster or vector data processing.
...
First, the data were read into the GRASS GIS environment using 'r.in.gdal' module and metadata of the Landsat TM scenes were checked up by GDAL \cite{GDAL} via ‘gdalinfo’ (Fig. \ref{Fig-2}, left). The metadata show general cartographic information of the scenes: coordinate system (UTM, zone 50N, WGS84 datum), resolution in pixel size (30 m), corner coordinates in WESN format. The next step included using a ‘g.region’ module (Fig. \ref{Fig-2}, right) which shows the settings of the current geographic region and applies the boundary definitions for the display monitor (module ‘d.mon’). The information about each scene was then obtained from the ‘r.info’ module (Fig. \ref{Fig-2}, right), which shows the general statistics on the raster image. The next step includes creating colour composites that were generated from the GRASS GIS project (Fig. \ref{Fig-3}, left) after the raster data were listed using module ‘g.list rast’ and necessary Landsat TM band triplets were selected and visualised using a sequence of the following modules (Fig. \ref{Fig-3}, right): 
...
Several GRASS GIS modules were used for auxiliary yet necessary steps. For instance,  the ‘d.mon’ module was used for display of the monitor in the GRASS system, the ‘d.erase’ -- to clean up the previous image on the display, and the ‘d.rgb’ module -- for combination of the RGB triplets and their overlay in the active graphics frame. The labels were added using manually annotated written expressions by means of ‘d.text’ module that associates the representation entities, i.e., names and titles on the maps. The resulting combinations of the Landsat TM scenes are visualised on Fig. \ref{Fig-4}. The topographic mapping is presented on the basis of the available DEM (Fig. \ref{Fig-5}, right) using the GRASS GIS script consisting of several following modules (Fig. \ref{Fig-5}, left). 
...
The DEM data serve for modelling the generated topographic variables, such as slopes, which allows a realistic representation of the Earth's surface. Modelling topographic shapes (Fig. \ref{Fig-6}, left) shows steeper and gentler slopes which was used for environmental monitoring. Thus, the highly weathered materials and boulders along the valley slopes and high-intensity rainfalls can lead to the hazardous geologic events, such as landslide occurrences in the landforms with steep slopes. The hill-shaded DEM of Qingdao area was used to study the most important geomorphometric parameters, such as slope, aspect of the land surface. Slope is one of the most important attributes and variables in the landslide analysis and geomorphology. 

The presence of slope is a prerequisite for the landslide occurrences, because gravitational force is tangential to the surface is required to generate the shearing stress. The steep slopes of the landforms are always prone to landslide occurrences that affect agriculture regions and crops. As can be seen (Fig. \ref{Fig-6}, left), the study area of Shandong Province presents the following landform types: a dissected terrain comprising moderately elevated hills (10°-17°, blue coloured areas), steep slopes (17°-45°, bright purple coloured areas), gentle lopes (0°-3°, light green coloured areas), moderate hills (3°-6°, cyan coloured areas), narrow-deep valleys formed by the weathering and erosion of the local river system (7°-10°, light blue coloured areas) flowing in eastwards into the Yellow Sea. 

Following slope modelling, an aspect of the geographical orientation of the compass was plotted (Fig. \ref{Fig-6}, right) aimed at visualizing WESN orientation of the slopes in the study area. Modelling and cartographic visualising of the land surface topography and bathymetry is useful to asses the processes of runoff and erosion which depend on the topographic morphometrics. Modelling of slope, aspect, profile curvature and tangential curvature was performed using following GRASS GIS code: ‘r.slope.aspect elevation=lsat7\_2009\_DEM slope=Qslope aspect=Qaspect pcurvature=Qpcurv tcurvature=Qtcurv’. Other cartographic elements were plotted by the following code snippets:
...
The calculations applied from the existing algorithms developed by \cite{NetelerMitasova2008} show temperatures (Landsat TM7). These were computed under the assumption of calibration of the physical constants and physical properties of the atmosphere. First, the correction (gain/bias) values are applied to the thermal channel to receive spectral radiances using the existing algorithms \cite{Barsietal2003}; \cite{NetelerMitasova2008}. The re-calculated pixel values were then converted to the absolute temperature in Kelvin (Fig. \ref{Fig-8} and Fig. \ref{Fig-9}, left). Third, the results were re-calculated to a degree Celsius temperature map by ‘r.mapcalc’, (Fig. \ref{Fig-8} and Fig. \ref{Fig-9}). 
...
The resulting land surface temperature maps ((Fig. \ref{Fig-8} and Fig. \ref{Fig-9}), right) show an average land surface temperature in Celsius degrees with a range of pixels with lower temperatures (water areas in lakes and forests) and higher temperatures (urban areas). A yellow-to-red colour tables was applied for the visualised map for the 19 May 2009 and 10 September 2004. This map shows the distributed emitted thermal radiation in degree Celsius in general colour scale for positive temperatures from 0° to 40°. 
...
Technical flexibility of GRASS GIS is demonstrated by four case studies: 1) colour composites; 2) topographic DEM visualization and modelling its derivatives (slope and aspect); 3) plotting correlation of spectral channels; 4) modelling land surface temperature based on the Landsat TM Band short-wave Infrared (SWIR) 6 with wavelengths of 10.40-12.50 µm and with 30 m spatial resolution. Using GRASS GIS, several scripts applying modules for raster data processing was applied and output maps visualised (e.g. Fig. 5, 8 and 9). GRASS GIS scripting console was used for testing and integrating modules (Fig. 2 and 3). The results shown that the spatio-temporal change in land surface temperature and topographic situation correlated in study region and demonstrated different patterns over urbanised areas (cities) and natural areas (forests, bare lands and water). This highlights that information on land cover changes, as derived from the satellite remote sensing data, can be a very useful indicator for environmental monitoring. Besides the environmental aspects, this study also points at technical aspects of the GRASS GIS and presents various modules used in a combined way for mapping and cartographic visualisation using scripting technique.

Global Visualization Viewer (GloVis) USGS external data source was used for downloading Landsat TM scenes through selecting study area by coordinates, viewing metadata and image capture.  Presented illustrations of the GRASS GIS data analysis aim to comparatively overlay raster maps visualising models from multiple different disciplines such as topography and geomorphology (Fig. 5 and 6), environment and climate change (Fig. 8 and 9) as well as remote sensing data and technical aspects of Landsat TM bands (Fig. 4 and 7). In addition to the integration of geography, GIS and machine learning (IT), the application of GRASS GIS scripting approach for cartographic mapping aims to enable advanced geospatial analysis. 

Presented paper demonstrated functionality of GRASS GIS software which proved to be excellent cartographic software for geospatial modelling that includes a wide variety of tasks: advanced raster data conversion and importing, geodetic warping and re-projections, statistical data processing, cartographic design, geomorphometric analysis and modelling, visualisation and project management. The integrated use of DEM, satellite images and GRASS approach provided important information regarding the environmental situation in eastern China which was assessed for changes during five-year time span. As a special remark it should be mentioned that GRASS GIS gives support in a number of ways. The GRASS GIS has an extensive help function which enables cartographer to select necessary module and information about the meaning of flags and arguments. The GRASS GIS has various resources as tutorial dataset which assist in mastering GIS, including vector and raster free data and documentation for native scripting language implemented in GRASS GIS. 
...
This study used a novel machine learning approach of GRASS GIS that enables processing spatial datasets by scripts from the console without GUI. The data were acquired by the GloVis USGS and included Landsat TM images with 5-years span (19 May 2009 and September 10 2004) used to identify and monitor environmental and topographic parameters in the Shandong Province region, north-eastern China. The environmental parameters of the mapped region show high correspondence and increases according to the topographic situation and land use types: the land surface temperature has lower values over the water areas and forests, while and higher values in urban areas. During the assessed time span of 5 years, there were notable changes in land surface temperature, from the 2004 and 2009 seasons which point at the gradually increased urbanisation in this area. In this study changes in land surface temperature and geomorphometric parameters (slope, aspect) were quantified using a set of GRASS GIS modules, as described in section 3 Methods and Samples, with presented screenshots of scripts used for plotting maps from the console. 

The ultimate goal of cartographic data processing is to reduce time and labour workflow and to increase the level of automation for more accurate and rapid data processing. On the other side, the remote sensing also requires increased machine-based data processing in order to obtain objective results in data modelling and classification. As a response to these needs, GRASS GIS presents advantageous techniques in many aspects which can be summarised as follows:

\begin{enumerate}
 	\item GRASS GIS shell scripting technique is proposed by console (in this study used XCode), and the scripts, once written, can also be repeated with small modifications and applied to process similar data in relevant research. Thus, the biggest advantage of scripts consists in repeatability.
	\item As demonstrated, GRASS GIS has relevant modules specially designed for raster data processing: ‘d.rast’, ‘r.colors’, ‘r.mapcalc’, ‘d.rgb’. Besides that, special modules can be used for vector data processing and data reading ('v.in.region', 'd.vect', 'v.info' and  ‘r.in.gdal’). Thus, another advantage is flexibility of data formats and processing.
	\item The environmental analysis presented in this study was done using remote sensing data using information derived from the satellite images. Therefore, it can be extended to many fields in applications in similar environmental or urban studies using Landsat TM/ETM+ images. Thus, another advantage of the GRASS GIS can be formulated as broad thematic applications (urban mapping, environmental analysis, etc).
\end{enumerate}
